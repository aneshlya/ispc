...
description: Analyzes LLVM trunk nightly build failures, categorizes them (ISPC regression, lit test failures, LLVM API changes, LLVM regressions), and proposes fixes. Invoked automatically by the llvm-trunk-failure-analyzer workflow or manually via the /analyze-llvm-trunk skill.
model: inherit
