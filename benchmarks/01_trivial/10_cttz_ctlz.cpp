...
// Platform-specific trailing zeros count
template <typename T> static T count_trailing_zeros(T value) {
    ...
    if constexpr (sizeof(T) == 8) {
        return _BitScanForward64(&index, value) ? index : sizeof(T) * CHAR_BIT;
    } else {
        return _BitScanForward(&index, value) ? index : sizeof(T) * CHAR_BIT;
    }
#else
    if constexpr (sizeof(T) == 8) {
...
// Platform-specific leading zeros count
template <typename T> static T count_leading_zeros(T value) {
    ...
    unsigned long index;
    if constexpr (sizeof(T) == 8) {
        return _BitScanReverse64(&index, value) ? (sizeof(T) * CHAR_BIT - 1 - index) : sizeof(T) * CHAR_BIT;
    } else {
        return _BitScanReverse(&index, value) ? (sizeof(T) * CHAR_BIT - 1 - index) : sizeof(T) * CHAR_BIT;
    }
#else
    if constexpr (sizeof(T) == 8) {
