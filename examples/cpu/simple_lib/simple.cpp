...
int main() {
    ...
    std::vector<const char *> args1 = {"ispc", "simple.ispc", "--target=host", "-O2", "-o",
                                       "simple_ispc.o", "-h", "simple_ispc.h"};

    int result = ispc::CompileFromArgs(static_cast<int>(args1.size()), const_cast<char **>(args1.data()));
    ...
    std::vector<const char *> args2 = {"ispc", "simple.ispc", "--target=host",  "-O0", "--emit-asm", "-o",
