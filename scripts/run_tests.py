            ...
            if host.is_windows():
                ...
                if target.is_xe():
                    if os.getenv("CLSH_DRIVER"):
                        ...
                        cc_cmd = "%s -O2 -I tests -I%s/usr/include -L%s/usr/lib/x86_64-linux-gnu -lze_intel_gpu  \
                                %s %s -DTEST_SIG=%d -DTEST_WIDTH=%d -o %s" % \
                                (options.compiler_exe, neo_driver, neo_driver, gcc_arch, add_prefix("tests/test_static_l0.cpp", host, target), match, width, exe_name)
                    else:
                        exe_name = "%s.run" % os.path.basename(testname)
