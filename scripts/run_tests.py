            ...
            if host.is_windows():
                ...
                if options.jit_mode:
                    ...
                    cc_cmd = "%s /Itests /I..\\src\\include /Zi /nologo /DTEST_SIG=%d /DTEST_WIDTH=%d %s /LIBPATH:%s ispc.lib /Fe%s" % \
                             (options.compiler_exe, match, width, add_prefix(test_file, host, target), jit_lib_path, exe_name)
                else:
...
def get_jit_library_path(host, options):
    if options.jit_lib_path:
        ...
        # Auto-discover: <path_to_ispc>/../lib
    ...
    if host.is_windows():
        ...
        if not os.path.exists(lib_file):
            error("ISPC library file not found: %s\n" % lib_file, 1)
    else:
        # On Unix systems, check for both static and shared libraries
        static_lib = os.path.join(lib_path, "libispc.a")
        shared_lib = os.path.join(lib_path, "libispc.so")
        if not os.path.exists(static_lib) and not os.path.exists(shared_lib):
            error("ISPC library file not found. Checked:\n  %s\n  %s\n" % (static_lib, shared_lib), 1)
