 ...
 */
bool Module::writeDeps(const CompilationResult& result, Output &CO) {
...
bool Module::writeDevStub(const CompilationResult& result) {
    ...
    result.symbolTable->GetMatchingFunctions(lIsExported, &exportedFuncs);
...
bool Module::writeHostStub(const CompilationResult& result) {
...
void Module::writeHeader(const CompilationResult& result, FILE *f) {
    ...
    result.symbolTable->GetMatchingFunctions(lIsExported, &exportedFuncs);
    result.symbolTable->GetMatchingFunctions(lIsExternC, &externCFuncs);
    ...
    for (int i = 0; i < (int)result.exportedTypes.size(); ++i) {
        if (const StructType *st = CastType<StructType>(result.exportedTypes[i].first)) {
            exportedStructTypes.push_back(CastType<StructType>(st->GetAsUniformType()));
        } else if (const EnumType *et = CastType<EnumType>(result.exportedTypes[i].first)) {
            exportedEnumTypes.push_back(CastType<EnumType>(et->GetAsUniformType()));
        } else if (const VectorType *vt = CastType<VectorType>(result.exportedTypes[i].first)) {
...
bool Module::writeHeader(const CompilationResult& result) {
...
bool Module::writeDispatchHeader(const CompilationResult& result, DispatchHeaderInfo *DHI) {
    FILE *f = DHI->file;
...
bool Module::writeNanobindWrapper(const CompilationResult& result) {
