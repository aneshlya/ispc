...
// CHECK: JIT engine isolation test: SUCCESS
...
int main() {
    ...
    // Test 1: Basic JIT compilation - create a simple ISPC program and compile it to JIT
    {
        ...
        std::vector<std::string> args = {"ispc", "jit_test.ispc", "--target=host"};
    ...
    // Test 4: JIT engine isolation test
    {
        std::vector<std::string> engine_args1 = {"ispc", "--target=host", "-O2"};
        std::vector<std::string> engine_args2 = {"ispc", "--target=host", "-O0"};

        auto engine1 = ispc::ISPCEngine::CreateFromArgs(engine_args1);
        auto engine2 = ispc::ISPCEngine::CreateFromArgs(engine_args2);

        bool engines_created = engine1 && engine2;
        bool engines_compiled = false;

        if (engines_created) {
            int r1 = engine1->CompileFromFileToJit("jit_test.ispc");
            int r2 = engine2->CompileFromFileToJit("jit_test.ispc");
            engines_compiled = (r1 == 0 && r2 == 0);
            
            if (engines_compiled) {
                // Verify both engines have independent function access
                auto func1 = engine1->GetJitFunction("double_array");
                auto func2 = engine2->GetJitFunction("double_array");
                engines_compiled = (func1 != nullptr && func2 != nullptr);
            }
        }

        if (engines_created && engines_compiled) {
            std::cout << "JIT engine isolation test: SUCCESS\n";
        } else {
            std::cout << "JIT engine isolation test: FAILED\n";
            all_tests_passed = false;
        }
    }
